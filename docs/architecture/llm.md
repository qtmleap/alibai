...
`history` はそのNPCとの会話履歴のみです。他NPCとの会話は混ざりません。プレイヤーの発話は必ず `user` ロールに閉じ込め、system 側へ混入させません。
...
`npcLied` はプレイヤーには見せず、リザルトの解析に使います。`suggestedQuestions` は「AIが会話内容から次の質問候補を提示する」UI に流します。スマホの自由入力だけでは操作コストが高いためです。
